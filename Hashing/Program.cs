...
namespace Hashing
{
    class Program
    {
        ...
        public string HammingHash(string currentFile, string hashAlgo, UInt64 hashOfSourceImage)
        {
            ...
            Int64 HammingDist = Hamming(hashOfSourceImage, filehash);

            string actualCurrentFile = currentFile.Remove(0, Directory.GetCurrentDirectory().Count() + 1);
            string returnstr = actualCurrentFile + ";"+HammingDist.ToString();
        ...
        static void Main(string[] args)
        {
            ...
            ConcurrentBag<string> hashcb = new ConcurrentBag<string>();
            ...
            */
            ...
            foreach (var currentFile in files)
            {
                ...
                TextWriter TW = new StreamWriter(actualCurrentFile + ".txt"); // Create a txt file for each jpg file in the current dir
                ...
                if (TypeOfHash == "aHash")
                {
                    currentHash = aHash(bm);
                }
                else if (TypeOfHash == "dHash")
                {
                    currentHash = dHash(bm);
                }
                else if (TypeOfHash == "gHash")
                {
                    currentHash = gHash(bm);
                }
                else if (TypeOfHash == "pHash")
                {
                    currentHash = pHash(bm);
                }
                bm.Dispose();
                ...
                Parallel.ForEach(files, (search) =>
                {
                    if (currentFile != search)
                    {
                        ...
                        hashcb.Add(p.HammingHash(search, TypeOfHash, currentHash));
                ...
                */
                ...
                // clear the bag
                while (hashcb.Count > 0)

                {

                    string element;

                    if (hashcb.TryTake(out element))
